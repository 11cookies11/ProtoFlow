  ...
  <div class="grid grid-cols-12 gap-6">
    <section class="col-span-12 lg:col-span-5 bg-white rounded-xl border border-slate-200 shadow-sm p-5 space-y-4">
      <div class="flex items-center justify-between">
        <h2 class="text-sm font-bold text-slate-800">YAML UI 编辑器</h2>
        <span class="text-xs text-slate-400">实时渲染</span>
      </div>
      <textarea
      ...
      <div v-if="store.parseError" class="text-xs text-rose-600 bg-rose-50 border border-rose-200 rounded-lg p-3">
        <div class="font-bold">解析错误</div>
        <div>{{ store.parseError.message }}</div>
        <div v-if="store.parseError.path" class="mt-1">路径: {{ store.parseError.path }}</div>
        <div v-if="store.parseError.line">行 {{ store.parseError.line }} 列 {{ store.parseError.column }}</div>
      </div>
    </section>
    ...
    <section class="col-span-12 lg:col-span-7 space-y-6">
      <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
        ...
        />
        <div v-else class="text-sm text-slate-400">等待 YAML 配置...</div>
      </div>
