...
<template>
  <section class="page">
          <header class="page-header spaced scripts-header">
            <div>
              <h2 class="scripts-title">
                {{ t('header.scripts.title') }}
                <span class="chip">{{ scriptFileName }}</span>
                <span class="badge">{{ t('badge.readOnly') }}</span>
              </h2>
            ...
            <div class="header-actions">
              ...
              <button class="btn btn-outline" @click="saveYaml">
                <span class="material-symbols-outlined">save</span>{{ tr('保存') }}</button>
            </div>
            <input
          ...
          <div class="scripts-grid">
            <div class="panel editor">
              <div class="panel-title bar">
                <span class="material-symbols-outlined">code</span>{{ tr('DSL 编辑器') }}<div class="panel-actions">
                  ...
                  <button class="icon-btn" :title="tr('\u590d\u5236')" @click="copyYaml">
                    <span class="material-symbols-outlined">content_copy</span>
                  </button>
                  <button class="icon-btn" :title="tr('\u641c\u7d22')" @click="searchYaml">
            ...
            <div class="scripts-side">
              <div class="panel stack">
                <div class="panel-title simple">{{ tr('执行控制') }}</div>
                ...
                <div class="button-grid">
                  <button
                    ...
                    @click="runScript"
                  >
                    <span class="material-symbols-outlined">play_arrow</span>{{ tr('运行') }}</button>
                  <button
                    ...
                    @click="stopScript"
                  >
                    <span class="material-symbols-outlined">stop</span>{{ tr('停止') }}</button>
                </div>
                <div class="progress-block">
                  <div class="progress-row">
                    <span>{{ tr('当前步骤:') }}<strong>{{ scriptState }}</strong></span>
          ...
          <div class="panel log-panel">
            <div class="panel-title bar">
              <span class="material-symbols-outlined">terminal</span>{{ tr('运行日志') }}<div class="panel-actions">
                <button class="icon-btn" :title="tr('\u6e05\u7a7a\u65e5\u5fd7')" @click="clearScriptLogs">
                  <span class="material-symbols-outlined">block</span>
                </button>
                <button class="icon-btn" :title="tr('\u6eda\u52a8\u5230\u5e95\u90e8')" @click="scrollScriptLogsToBottom">
