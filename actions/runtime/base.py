...
class RuntimeTaskValidationError(ValueError):
    pass
...
@dataclass
class RuntimeTaskBase:
    name: str
    schema: Dict[str, Any] | None = None

    def run(self, task: Dict[str, Any] | None, channels: Dict[str, Any], logger) -> Any:
        payload = self.validate_task(task)
        return self.execute(payload, channels, logger)

    def execute(self, task: Dict[str, Any], channels: Dict[str, Any], logger) -> Any:
        raise NotImplementedError()

    def validate_task(self, task: Dict[str, Any] | None) -> Dict[str, Any]:
        if task is None:
            task = {}
        if not isinstance(task, dict):
            raise RuntimeTaskValidationError("task must be a mapping")
        ...
        for key in required:
            if key not in normalized:
                raise RuntimeTaskValidationError(f"missing required field: {key}")

        if not allow_extra:
            allowed = set(required) | set(optional) | set(types)
            extra = [k for k in normalized.keys() if k not in allowed]
            if extra:
                raise RuntimeTaskValidationError(f"unknown fields: {', '.join(extra)}")
        ...
        for key, type_spec in types.items():
            ...
            if expected is not None and not isinstance(value, expected):
                raise RuntimeTaskValidationError(
