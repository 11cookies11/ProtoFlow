...
- Supported actions: `modbus_read` / `modbus_write`
...
> Note: this example runs with the registered `modbus_read` / `modbus_write` actions.
```yaml
vars:
  retries: 0
channels:
...
- Note: `meter_start/meter_add/meter_stop` are still reserved and not implemented in the current DSL runner.
