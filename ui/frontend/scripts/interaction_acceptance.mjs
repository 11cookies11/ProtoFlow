...
function run() {
  const failures = []
  const proxyMonitorEnabled = process.env.PROTOFLOW_ENABLE_PROXY_MONITOR === '1'

  if (proxyMonitorEnabled) {
    const toolbar = read('src/components/proxy/ProxyCaptureToolbar.vue')
    expectIncludes(
      toolbar,
      "defineEmits(['update:search-keyword', 'resume-capture', 'open-settings'])",
      'ProxyCaptureToolbar emits',
      failures
    )
    ...
    expectIncludes(
      footer,
      "defineEmits(['page-first', 'page-prev', 'page-next', 'page-last', 'export'])",
      'ProxyCaptureFooter emits',
      failures
    )
    ...
    expectIncludes(
      ...
      'ProxyMonitorView footer last page binding',
      failures
    )
